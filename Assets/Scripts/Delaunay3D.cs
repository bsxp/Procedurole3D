...
public class Delaunay3D {
    ...
    void Triangulate() {
		...
		// Update super-tetrahedron dimensions to accurately encompass all values
		...
		// Construct vertices of super-tetrahedron
		...
		// Buid super-tetrahedron
        Tetrahedra.Add(new Tetrahedron(p1, p2, p3, p4));

		// For each vertex inside of the super-tetrahedron
        foreach (var vertex in Vertices) {
            List<Triangle> triangles = new List<Triangle>();

            foreach (var t in Tetrahedra) {

				// If the circumcircle contains the vertex we're looking at
                if (t.CircumCircleContains(vertex.Position)) {

					// The tetrahedron is bad
                    t.IsBad = true;

					// Add four new triangles (sides of a tetrahedron) to our list
			...
			// For each triangle in our new list of possible inspectable triangles (faces of a tetrahedron)
            for (int i = 0; i < triangles.Count; i++) {
				// Look at every other triangle in our new list of possible inspectable triangles
                for (int j = i + 1; j < triangles.Count; j++) {

					// If the triangle is almost equal to another triangle
                    if (Triangle.AlmostEqual(triangles[i], triangles[j])) {

						// Both triangles are bad
			...
			// Remove bad tetrahedrons (aka our super-tetrahedron)
            ...
            foreach (var triangle in triangles) {
				...
				// Now, for every valid triangle remaining, we should create a tetrahedron from the triangles vertices and the vertex we're currently looking at
		...
		Edges.RemoveAll((Edge edge) => edge.U.Equals(p1) || edge.U.Equals(p2) || edge.U.Equals(p3) || edge.U.Equals(p4) ||
				edge.V.Equals(p1) || edge.V.Equals(p2) || edge.V.Equals(p3) || edge.V.Equals(p4));
